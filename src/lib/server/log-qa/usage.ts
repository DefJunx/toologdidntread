...
export const getDailyCount = async (d1: D1Database, userId: string, utcDay: string): Promise<number> => {
	const db = getDb(d1);
	const rows = await db
...
export const incrementDailyCount = async (d1: D1Database, userId: string, utcDay: string): Promise<number> => {
	const db = getDb(d1);
	await db
		.insert(logQaUsage)
		...
		.onConflictDoUpdate({
			...
			set: { questionCount: logQaUsage.questionCount + 1, updatedAt: new Date() }
		});
